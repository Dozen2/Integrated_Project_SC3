...
const SESSION_KEY = "SaleItem-FilterBrand";
...
const getSessionFilterBrands = () => {
  const raw = sessionStorage.getItem(SESSION_KEY);
  return raw || "";
};

// Set session storage value
const setSessionFilterBrands = (value) => {
...
// Watch for prop changes (for compatibility if still needed)
watch(
...
const emitBrandFilter = () => {
  ...
  filterBrands.value = filterValue;
  setSessionFilterBrands(filterValue);
...
onMounted(async () => {
  ...
  const sessionValue = getSessionFilterBrands();
  const initialValue = sessionValue || props.initialFilterBrands || "";
  
  if (initialValue) {
    filterBrands.value = initialValue;
    selectedBrandList.value = initialValue
      .split(",")
      .filter((brand) => brand.trim() !== "");
  }
